...
io.on("connection", (socket) => {
	...
	socket.on("join", (params, callback) => {
		...
		if (
			numClients[params.room] == undefined ||
			numClients[params.room] == 0
		) {
			console.log(
				`Creating room ${params.room} and emitting room_created socket event`
			);
			socket.join(params.room);
			socket.emit("room_created", params.room);
		} else if (numClients[params.room] == 1) {
			console.log(
				`Joining room ${params.room} and emitting room_joined socket event`
			);
			socket.join(params.room);
			...
			flg = 1;
			console.log(
				`Can't join room ${params.room}, emitting full_room socket event`
			);
		...
		if (flg == 0) {
			...
			socket
				...
				.emit(
					"user-connected",
					params.name /*, numP: numClients[params.room]*/
				);
	...
	socket.on("noCheat", () => {
		socket.to(users_in.get(Ussers[socket.id])).emit("cheating", {
			name: users_id.get(Ussers[socket.id]),
			num: 0,
		});
	});
	...
	socket.on("alert_users", () => {
		socket
			.to(users_in.get(Ussers[socket.id]))
			.emit("alert_user", users_id.get(Ussers[socket.id]));
	});
	...
	socket.on("disconnect", () => {
		...
		socket.to(users_in.get(Ussers[socket.id])).emit("user-disconnected", {
			name: users_id.get(Ussers[socket.id]),
			numP: numClients[users_in.get(Ussers[socket.id])] - 1,
		});
		users_in.delete(Ussers[socket.id]);
		users_id.delete(Ussers[socket.id]);
		delete Ussers[socket.id];

		if (numClients[ROOMID] == 0) numClients[ROOMID] = undefined;
		else numClients[ROOMID]--;
		console.log(numClients);
	...
	function compile(code, input, language) {
		...
		var program = {
			...
			clientSecret:
				"13edd9c3d46c9ec50ae774c6fa5bcc65c24117b87be1b4e19d7edcaf4deb6225",
		};
		request(
			{
			...
			function (error, response, body) {
				...
				if (error) socket.emit("output", error);
				else socket.emit("output", body);
